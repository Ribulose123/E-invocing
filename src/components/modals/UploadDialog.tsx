...
// API expects InvoicedQuantity (and other line numerics) as number. Extract number from "31 DAYS" etc.
 ...
 *   3. Missing opening `{"` (Excel truncates it) → prepend `{"` and close open braces
 *   4. Missing opening `{` only → prepend `{` and close open braces
 */
    ...
    try {
      ...
      if (res.ok) {
        ...
        try {
          ...
          if (errJson.status_code === 422 && errObj && typeof errObj === 'object' && !Array.isArray(errObj)) {
            const lines = Object.entries(errObj).map(([field, m]) => `${field}: ${m}`);
            msg = errJson.message + '\n\n' + lines.join('\n');
          } else {
            msg = errJson.message || JSON.stringify(errObj) || text;
          }
        } catch {}
        setUploadStatus('error');
